...
def execute_trade_signal(symbol, prediction_prob, current_qty, df):
    ...
    if time.time() - last_trade_times.get(symbol, 0) < COOLDOWN_SECONDS:
        return
    ...
    if prediction_prob >= BUY_THRESHOLD and current_qty <= 0:
        ...
        try:
            ...
            logger.info(f"✅ API Response: {order.id} | Status: {order.status}")
            last_trade_times[symbol] = time.time()
            save_trade_state(last_trade_times)
        except Exception as e:
            logger.error(f"❌ CRITICAL REJECTION for {symbol}: {str(e)}")
            last_trade_times[symbol] = time.time()
            save_trade_state(last_trade_times)
    ...
    # SELL Logic
        ...
        try:
            ...
            logger.error(f"❌ CRITICAL REJECTION for {symbol}: {e}")
